...
from src.todo_app.ui.cli import app
...
def test_add_command():
    ...
    import src.todo_app.ui.cli as cli_module
...
def test_list_command():
    ...
    try:
        ...
        assert "List Tasks" in result.stdout  # Table title (i18n version)
    finally:
        # Restore original service
        cli_module.service = original_service
...
def test_list_command_empty():
    ...
    try:
        result = runner.invoke(app, ["list-tasks"])
        assert result.exit_code == 0
        assert "no_tasks" in result.stdout  # i18n version of "No tasks found"
    finally:
        # Restore original service
        cli_module.service = original_service
