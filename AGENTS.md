...
The project includes an MCP (Model Context Protocol) server that exposes all Spec-Kit Plus commands as MCP prompts. This allows Claude Code to access the complete SDD workflow directly.
...
The MCP server is automatically configured when Claude Code starts in this project directory. The configuration is in `.mcp.json`.

### **Usage**

1. Start Claude Code in the project root directory
2. The MCP server will automatically connect
3. Use commands like `/sp.specify`, `/sp.plan`, etc. as MCP prompts
